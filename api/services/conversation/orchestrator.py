...
class ConversationFlow:
    ...
    async def handle_interactive_reply(self, phone: str, reply_id: str) -> None:
        ...
        if step == STEP_AWAITING_CLIENT:
            await self._ask_priority(phone, state, client_id=reply_id)
    ...
    async def on_buffer_close(self, phone: str, messages: list[str]) -> None:
        ...
        state: dict[str, Any] = {
            ...
            "external_department_id": (
                str(worker.external_department_id) if worker.external_department_id else None
            ),
        }
    ...
    async def _handle_client_reply(self, phone: str, text: str) -> None:
        ...
        if len(results) == 1:
            await self._ask_priority(phone, state, client_id=results[0].id)
        elif len(results) == 0:
            await self._ask_priority(phone, state, client_id=None)
        else:
            options = [{"id": r.id, "title": r.name} for r in results[:10]]
            if len(options) <= 3:
    ...
    async def _finish(self, phone: str, state: dict[str, Any], priority: str) -> None:
        ...
        if not created_by:
            logger.error(
                "No se puede crear el ticket para %s: el worker no tiene external_user_id "
                "configurado todavía (stopgap manual, ver CLAUDE.md Pendientes).",
                phone,
            )
        ...
        ticket_id = await self._ticket_system.create_ticket(
            title=state["title"],
        ...
        department_id = state.get("external_department_id")
        if department_id:
            await self._ticket_system.add_department(
                ticket_id=ticket_id, department_id=department_id, actor_id=created_by
            )

        await self._meta.send_text(phone, f"Ticket creado: #{ticket_id}")
